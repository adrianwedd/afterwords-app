...
`HealthMonitor` polls every 5s when running, every 2s when starting. State transitions are driven entirely by poll results — CLI commands are fire-and-forget, but every `notifyStartAttempt()`/`notifyStopAttempt()` call is **gated on the CLI accepting the command**: `startServer()`/`stopServer()`/`restartServer()` return `Bool` (path validation passed + process spawned — `Process.run()` executes synchronously before the command reports acceptance), and a refused or failed-to-spawn launch stays in the current state with the error shown, instead of stranding the UI in a `.starting` no process can ever satisfy. **Restart** fires `afterwords restart` *and*, when accepted, calls `notifyStartAttempt()`, so it also enters `.starting`. The `.error → start` edge uses the same `notifyStartAttempt()` path as manual Start/Restart. (The silent auto-start does not traverse `.error → start`; it fires from the `.stopped` branch on the first poll failure (HealthMonitor.swift), with the same acceptance gate.)
