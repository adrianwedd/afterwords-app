...
- **State Management**: The app uses `@StateObject` and `@EnvironmentObject` to inject core services (`CLIExecutor`, `HealthMonitor`, `SamplePlayer`) into the view hierarchy.
...
- **UI State vs OS State**: If an operation fails (e.g., registering for Launch at Login), the app must revert the UI toggle to match the actual OS state (re-entrancy guards are used for this).
